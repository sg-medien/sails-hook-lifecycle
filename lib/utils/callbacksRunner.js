...
runner.afterUpdate = function(context, record, criteria, oldValues, cb) {

  var fn = function(item, next) {

    if (item.length === 3) {
      item.call(context, record, oldValues, next);
    } else if (item.length === 4) {
      const finalCriteria = (criteria && _.isObject(criteria.where) && _.size(criteria.where)) ? criteria.where : null;

      item.call(context, record, oldValues, finalCriteria, next);
    } else {
      // legacy usage: if function takes 2 params, do not pass values or criteria
      item.call(context, record, next);
    }
  };
...
runner.beforeDestroy = function(context, criteria, cb) {
  ...
  var fn = function(item, next) {
    if (item.length === 1) {
      ...
      item.call(context, finalCriteria, next);
    }
  };
